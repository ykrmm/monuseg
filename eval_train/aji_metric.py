# Code in python of the AJI metrics for the MoNuSeg Challenge 
...
def Aggregated_Jaccard_Index_v1_0(gt_map=None, predicted_map=None):
    ...
    pr_list = pr_list(mslice[2:])# exclude 0
    # mark used nuclei by the number of uses (you can use any other criteria) 
    pr_list = mcat([pr_list, zeros(size(pr_list, 1), 1)])
    npredicted = numel(pr_list(mslice[:], 1))
    ...
    while not isempty(gt_list):    # loops over each gt
        fprintf(mstring('Processing object # %d \\n'), i)
        ...
        predicted_match = gt *elmul* predicted_map
        ...
        if nnz(predicted_match) == 0:
            ...
            gt_list(i).lvalue = mcat([])        # remove it from the list
            ...
            predicted_nuc_index = predicted_nuc_index(mslice[2:])        # exclude 0 from unique set
            # you can make it even faster if only one match is identified
            JI = 0
            for j in mslice[1:length(predicted_nuc_index)]:
                matched = 1 * (predicted_map == predicted_nuc_index(j))
                nJI = nnz(_and(gt, matched)) / nnz(_or(gt, matched))
                if nJI > JI:
            ...
            overall_correct_count = overall_correct_count + nnz(_and(gt, predicted_nuclei))
            union_pixel_count = union_pixel_count + nnz(_or(gt, predicted_nuclei))

            # remove used gt from the list
            gt_list(i).lvalue = mcat([])
            # move to next nuclei
            i = length(gt_list)
            # update the count for used predicted nuclei
            index = find(pr_list(mslice[:], 1) == best_match)
            pr_list(index, 2).lvalue = pr_list(index, 2) + 1
        
    

    # Find all unused nuclei
    unused_nuclei_list = find(pr_list(mslice[:], 2) == 0)
    # add all unmatched pixels left in the predicted map to union set
    for k in mslice[1:numel(unused_nuclei_list)]:
